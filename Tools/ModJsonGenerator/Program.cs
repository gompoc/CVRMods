...
namespace ModJsonGenerator
{
    internal static class Program
    {
        ...
        static int Main(string[] args)
        {
            ...
            var globalGameManagersPath = Path.Combine(args[1], "ChilloutVR_Data", "globalgamemanagers");
            Match match = ParseRegex.Match(Encoding.ASCII.GetString(File.ReadAllBytes(globalGameManagersPath)));

            ourMod.gameversion = $"{match.Groups[1]}";
        ...
        private static readonly Regex ParseRegex = new (@"([0-9]{4}r[0-9]+)");
